<template>
    <div ref="filemanager-container">
        ...
        <create-folder :active="showCreateFolder" :current="currentPath"
        ...
        <confirm-modal-multi-delete ref="confirmMultiDelete" :selected-files="selectedFiles"
        ...
        <div class="card relative" id="filemanager-manager">
            <div class="p-3 flex items-center justify-between border-b border-50">
                <div class="w-full flex flex-wrap">
                    <div class="w-1/2 flex flex-wrap justify-start">
                        <button @click="refreshCurrent" class="btn btn-default btn-small btn-primary text-white mr-3"
                        ...
                        <button v-if="buttons.select_multiple" type="button"
                                class="btn btn-default btn-primary text-white mr-3 flex items-center"
                                @click="multiSelecting=!multiSelecting">
                            <svg class="w-6 h-6 fill-current pt-1" xmlns="http://www.w3.org/2000/svg"
                                 viewBox="0 0 24 24">
                        ...
                        <button v-if="multiSelecting && selectedFiles.length > 0" type="button"
                                class="btn btn-default btn-small btn-danger text-white mr-3"
                                @click="openMultiDeleteModal">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                 aria-labelledby="delete" role="presentation">
                            ...
                            <div class="relative">
                                <div class="relative">
                                    <template v-if="showFilters">
                                        <select class="pl-search form-control form-input form-input-bordered w-full"
                                                v-model="filterBy" @change="filterFiles">
                                            <option value="">{{ __('Filter by ...') }}</option>
                                            <option v-for="(filter, key) in filters" :key="'filter_' + key"
                                                    :value="key">{{ key }}
                                            </option>
                                        </select>
                                    </template>
                        ...
                        <div class="relative w-2/3 max-w-xs">
                            <div class="relative">
                                <div class="relative">
                                    ...
                                    <input v-on:input="searchItems" v-model="search" dusk="filemanager-search"
                                           type="search" :placeholder="this.__('Search')"
            ...
            <UploadProgress ref="uploader" :current="currentPath"
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            if (localStorage.getItem('maia-filemanager-view')) {
                let viewS = localStorage.getItem('maia-filemanager-view');
                if (['grid', 'list'].includes(viewS)) {
                    this.view = viewS;
                } else {
                    localStorage.setItem('maia-filemanager-view', 'grid');
                }
            }
        ...
        methods: {
            ...
            viewAs(type) {
                this.view = type;
                localStorage.setItem('maia-filemanager-view', type);
            },
