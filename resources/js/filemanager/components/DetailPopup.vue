                ...
                <div class="flex flex-wrap">
                    <div class="w-3/5 box-preview flex justify-center" :class="cssType">
                        <template v-if="info.type === 'image'">
                            <ImageInfo :file="info" :preview="true"
                                       :css="'card relative w-full flex flex-wrap justify-center items-center overflow-hidden px-0 py-0'"/>
                        </template>
                    ...
                    <div class="w-2/5 bg-30 box-info flex flex-wrap">
                        <div class="info-data w-full">
                            <div class="info mx-4 my-3 flex flex-wrap items-center">
                                ...
                                <span class="value bg-white px-1 py-1 rounded-r"
                                      v-if="!editingName">{{ info.name }}</span>
                                <template v-if="buttons.rename_file">
                                    <svg v-if="!editingName" @click="editName" class="ml-1 cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="12" height="12">
                                        <path d="M12.3 3.7l4 4L4 20H0v-4L12.3 3.7zm1.4-1.4L16 0l4 4-2.3 2.3-4-4z"/>
                                    </svg>
                                    <template v-if="editingName">
                                        <input type="text" v-bind:ref="'inputName'"
                                               :style="{ 'width': nameWidth + 'px' }" v-model="nameNoExtension"
                                               class=" value px-1 py-1 rounded-r">
                                        <svg @click="rename" xmlns="http://www.w3.org/2000/svg"
                                             class="ml-1 cursor-pointer text-success fill-current" viewBox="0 0 20 20"
                                             width="12" height="12">
                                            <path d="M0 11l2-2 5 5L18 3l2 2L7 18z"/>
                                        </svg>
                                        <svg @click="editingName = !editingName" xmlns="http://www.w3.org/2000/svg"
                                             class="ml-1 cursor-pointer" viewBox="0 0 20 20" width="12" height="12">
                            ...
                            <div class="info mx-4 mt-6" v-if="info.url">
                                ...
                                <div class="flex flex-wrap items-stretch w-full mb-4 relative">
                                    <input type="text"
                                           class="flex-shrink flex-grow flex-auto text-xs leading-normal w-px flex-1 border border-70 rounded rounded-r-none px-1 relative"
                                           :value="info.url" disabled>
                                    <div class="flex -mr-px">
                                        <button class="copy flex items-center leading-normal bg-50 rounded rounded-l-none border border-l-0 border-70 px-3 whitespace-no-wrap text-grey-dark text-xs"
                                                v-copy="info.url" v-copy:callback="onCopy">{{ __('Copy') }}
                                        </button>
                                    </div>
                                </div>
                            </div>
