    ...
    <div class="py-3 px-6 border-50" v-else>
      <div class="flex">
        ...
        <div class="w-3/4 py-4">
          <p class="text-90">No seo settings fields have been defined.</p>
        </div>
      </div>
    </div>
  </loading-view>
  ...
  methods: {
    ...
    async update() {
      try {
        ...
        this.$toasted.show('Seo settings successfully updated', {
          type: 'success',
        });
    ...
    updateRequest() {
      return Nova.request().post('/nova-vendor/maia-seo/seo', this.formData);
    },
  },
  computed: {
    formData() {
